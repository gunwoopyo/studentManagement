...
public:
    ...
    Student* createObject(int studentID);  // 학번을 통해 학생 정보가 존재한다면 객체를 생성하는 함수
    void insertStudent(int studentID, QString name, QString major, QString year);  // 학생 객체를 생성해 링크드 리스트에 삽입하는 함수
    void insertStudent(int studentID, QString name, QString major, QString year, double GPA);
    void deleteStudent(int studentID); // 학생 객체를 링크드 리스트에서 삭제하는 함수
    void addCourse(Student* student, QString courseName); // 수강 과목 추가 함수
    ...
    void debugCourseList();  // 수강 과목 삽입 or 삭제 시 메모리 주소 디버그
    void deleteStudentDB(int studentID);  // 특정 학생 정보를 DB에서 삭제하는 함수
    void deleteCourseDB(int studentID, QString courseName);  // 특정 학생의 수강 과목을 DB에서 삭제하는 함수
};
#endif // MANAGEMENT_H
