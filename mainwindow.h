...
public slots:
    ...
    void on_deleteStudentTable_cellClicked(int row);    // 학생 삭제 셀 클릭
    void on_deleteStudentPushButton_clicked();          // 학생 삭제 버튼
    void searchClear();

    void searchTable(Student* headPointer);                                 // 학생 조회 테이블
    void searchStudentTable(int stnID, QString name, QString year, QString major);   // 학생 검색 테이블
    void applySearchTableRow(QTableWidget* studentTable, int row, Student* currentStudent);
    void applySearchTableRowColors(QTableWidget* studentTable);
    void totalStudentCount();

    void setSearchStudentInfo(int& studentID, QString& name, QString& year, QString& major);
